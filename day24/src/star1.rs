...
impl Position {
    fn forward_checked(&self, direction: Direction) -> Option<Position> {
...
pub fn main() {
    ...
    println!("{}", descent(&mut board, desired, queue, columns, rows));
}
...
fn descent(
    ...
    while let Some(pos_minute) = queue.pop_front() {
        ...
        if pos_minute.1 != last_minute {
            println!("onto minute {}", pos_minute.1);
        ...
        for direction in area_of_interest {
            if let Some(p) = pos_minute.0.forward_checked(direction) {
                if p == desired {
                    return pos_minute.1;
                }
