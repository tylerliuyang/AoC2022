...
fn filter_down(occupied: & HashSet<Stone>, stone: Stone, max_depth: usize) -> Option<Stone> {
  if max_depth < stone.1 {
    return None;
  }
...
pub fn main() {
  ...
  let mut i = 0;
  while let Some(stone) = filter_down(&occupied, SOURCE, max_depth) {
