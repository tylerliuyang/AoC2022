...
pub fn main() {
    ...
    for i in 0..MONKEY_COUNT {
        ...
        let items = input.split(" ").collect::<Vec<&str>>()
            .iter().skip(4).map(|s| -> i32 {s.strip_suffix(',').unwrap_or(s).parse().unwrap()})
            .collect::<Vec<i32>>();
        ...
        stdin().read_line(&mut input).unwrap();
        input = input.strip_suffix("\r\n").unwrap().to_string();
        let test: i32 = input.split(" ").collect::<Vec<&str>>().get(5).unwrap().parse().unwrap();

        let mut input = String::new();
        stdin().read_line(&mut input).unwrap();
        input = input.strip_suffix("\r\n").unwrap().to_string();
        let true_monkey: i32 = input.split(" ").collect::<Vec<&str>>().get(9).unwrap().parse().unwrap();

        let mut input = String::new();
        stdin().read_line(&mut input).unwrap();
        input = input.strip_suffix("\r\n").unwrap().to_string();
        let false_monkey: i32 = input.split(" ").collect::<Vec<&str>>().get(9).unwrap().parse().unwrap();
