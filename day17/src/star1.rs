use std::{collections::HashSet, cmp::max, io::stdin};
...
impl Shape {
    fn flip(&self) -> Shape {
        ...
        for point in &self.shape {
            shape.insert(Point {x: point.x, y: self.height - point.y});
        }
        Shape { shape, height: self.height }
    }
    ...
    fn move_to(&self, direction: Direction, amount:usize) -> Shape {
        ...
        for point in &self.shape {
        shape.insert(
            point.move_to(direction.clone(), amount)
        );
        }
        Shape { shape, height: self.height }
}
...
impl Point {
    fn move_to(&self, direction: Direction, amount: usize) -> Point {
      match direction {
        ...
        Direction::UP => {
            Point { x: self.x , y: self.y + amount }
        } 
        Direction::DOWN => {
            Point { x: self.x, y: self.y - amount }
        } 
...
fn try_move(shape: &mut Shape, direction: Direction, amount: usize, board: &HashSet<Point>) -> bool {
    if direction == Direction::LEFT {
        if shape.shape.iter().any(|point| {point.x == 0}) {return false}
    }
    if direction == Direction::DOWN {
        if shape.shape.iter().any(|point| {point.y == 0}) {return false}
    }
    if direction == Direction::RIGHT {
        if shape.shape.iter().any(|point| {point.x == 6}) {return false}
    }
    if shape.move_to(direction.clone(), amount).shape.iter().all(|point| {
...
pub fn main() {
    ...
    loop {
      ...
      match stdin().read_line(&mut input) {
        Ok(0) => {break}
        Ok(2) => {
            let elt = Shape {shape: shape.clone(), height: y-1}.flip();
          shapes.push(elt);
          shape = HashSet::new();
          y = 0;
        }
    ...
    for j in 0..2022 {
      ...
      let mut normalized_shape = shape.clone().move_to(Direction::RIGHT, 2).move_to(Direction::UP, max_height + 3);
